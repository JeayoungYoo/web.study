...
<body>
	<h1 align="center">${ member.name }님정보보기</h1>
	<br>
	<center>
		<%-- 
...
<tr><td colspan="2" align="center">
	...
	<a href="/testm/mdelete?userid=<%= m.getId() %>">탈퇴하기</a>
	<!-- 쿼리스트링 : 서블릿이름?이름=값&이름=값 -->
		...
		<form action="/testm/mupdate" method="post">
			<table width="600" cellspacing="5" bgcolor="#99ccff">
				...
				<tr>
					...
					<c:if test="${ member.gender eq 'M' }">
						<td><input type="radio" name="gender" value="M" checked>
							남 <input type="radio" name="gender" value="F"> 여</td>
					</c:if>
					<c:if test="${ memer.gender eq 'F' }">
				...
				<tr>
					<td>우편번호</td>
					<td><c:forTokens var="addr" items="${member.address }"
							delims="," varStatus="status">
							<c:if test="${status.index eq 0}">
								<tr>
									<td>우편번호</td>
									<td><input type="text" name="post" size="10"
										value="${addr}"></td>
								</tr>
							</c:if>
							<c:if test="${status.index eq 1}">
								<tr>
									<td>기본주소</td>
									<td><input type="text" name="address1" value="${addr}"></td>
								</tr>
							</c:if>
							<c:if test="${status.index eq 2}">
								<tr>
									<td>상세주소</td>
									<td><input type="text" name="address2" value="${addr}"></td>
								</tr>
							</c:if>
						</c:forTokens>
				<tr>
					<td colspan="2" align="center"><input type="submit"
						value="수정하기"> &nbsp; <a
						href="/testm/mdelete?userid=${ member.id }">탈퇴하기</a> <!-- 쿼리스트링 : 서블릿이름?이름=값&이름=값 -->
					</td>
				</tr>
			</table>
		</form>

		<br> <br> <a href="/testm/index.jsp">시작페이지로 이동</a>
	</center>
