...
<body>
	...
	<table align="center" cellpadding="10" cellspacing="0" border="1"
		...
		<tr align="center" valign="middle">
			<th colspan="2">${ board.boardNum }번글상세보기</th>
		</tr>
		<tr>
		...
		<tr align="center" valign="middle">
			<td colspan="2"><c:if test="${ !empty sessionScope.member }">
					<c:url var="replyform" value="/views/board/boardReplyForm.jsp">
						<c:param name="bnum" value="${ board.boardNum }" />
						<c:param name="bnum" value="${ currentPage }" />
					</c:url>
					<a href="${ replyform }"> [댓글달기] </a> &nbsp;&nbsp; 
         <c:if test="${ member.id == board.boardWriter }">
            ...
            <a href="${ bdelete }"> [글삭제] </a> &nbsp;&nbsp; 
				...
				</c:if> <c:url var="blist" value="${ blist }">
